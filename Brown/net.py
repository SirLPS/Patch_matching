...
from loader import create_loaders,args
...
class Model(nn.Module):
    ...
    def __init__(self):
        ...
        self.block = nn.Sequential(
            ...
            nn.Conv2d(64, 128, kernel_size=3, stride=1,padding=1, dilation=2, bias = False),   # stride = 2   
...
#        self.block.apply(weights_init)   # worse use the init 
...
def test(test_loader, model, epoch, logger_test_name):
    ...
    for batch_idx, (data_a, data_p, label) in pbar:
        ...
        out_a = model(data_a)
        out_p = model(data_p)
#        out_a, out_p = model(data_a, data_p)
...
if __name__ == '__main__':
    if args.which == 0:
        sets = ['liberty','yosemite', 'notredame']        
    if args.which == 1:
        sets = ['notredame','liberty', 'yosemite']
    if args.which == 2:
        sets = ['yosemite','liberty', 'notredame']


    print('Train:{:s}'.format(sets[0]))
    training_set = sets[0]
